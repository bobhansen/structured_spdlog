...
static std::string log_to_str(const std::string &msg, const std::array<spdlog::Field,N>& fields, const Args &... args)
{
...
TEST_CASE("json basic output", "[json_formatter]")
{
    ...
    REQUIRE(log_to_str("hello", NO_FIELDS, "", "MSG", "SRC", "LEVEL", "") == R"({"MSG":"hello", "SRC":"source.cpp:99", "LEVEL":"info"})");

    // Tests with regex outputs
    REQUIRE_THAT(log_to_str("hello", NO_FIELDS, "", "", "", "", "THREAD"), Matches(R"(\{"THREAD":[0-9]+\})"));
    // Time regex lifted from https://www.myintervals.com/blog/2009/05/20/iso-8601-date-validation-that-doesnt-suck/
    ...
    REQUIRE_THAT(log_to_str("hello", NO_FIELDS, "TM", "", "", "", ""), Matches(time_output_regex));
    ...
    // Fields with message
    REQUIRE(log_to_str("hello", fields, "", "MSG", "", "", "") == R"({"MSG":"hello", "f1":1, "f2":"two", "f3":3.000000})");
}
